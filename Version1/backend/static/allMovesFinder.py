...
def AllMovesFinder(chessBoard, square):
    ...
    if (isinstance(piece, Pawn)):
        ...
        if (not piece.isBlack):
            for x in range (4):
                if (MoveChecker(chessBoard, square, PositionsToNotation(file+fileShift[x], row+rowShift[x]),0)):
...
def AppendValidStraights(chessBoard, square, file, row, validMoves):
    ...
    while (checkedRow > row):
        if MoveChecker(chessBoard, square, PositionsToNotation(file, checkedRow), 0):
            while (checkedRow > row):
    ...
    while (checkedFile > file):
        if MoveChecker(chessBoard, square, PositionsToNotation(checkedFile, row), 0):
            while (checkedFile > file):
...
def AppendValidDiagonals(chessBoard, square, file, row, validMoves):
    ...
    while (checkedFile > 0 and checkedRow < 7):
        ...
        checkedRow += 1
    while (checkedFile < file):
        if MoveChecker(chessBoard, square, PositionsToNotation(checkedFile, checkedRow), 0):
            while (checkedFile < file):
